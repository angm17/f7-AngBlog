...
const app = new Framework7({
  ...
  methods: {
    ...
    addComment: function(comment, callback){
      //temporal id, to make the frontend work
      comment.id = comments.length + 1;
      comments.push(comment);
      callback(null, comment.id)
    },
...
const $$ = Dom7;
const postTemplate = `{{#each posts}}
<div class="card post">
  <div class="card-header text-color-orange">{{title}}</div>
  <div class="card-content card-content-padding text-align-justify postBody">{{body}}</div>
  <div class="card-footer">
      <p><a href="#" class="link">{{comments id}}</a></p>
      <a href="/post/{{id}}" class="link">Read more</a>
  </div>
</div>
{{#unless @last}}<hr class="padding-horizontal">{{/unless}}
{{else}}
<p class="text-align-center">No posts to show!</p>
{{/each}}
`;

Template7.registerHelper('comments', function (id){
  commentstxt = app.methods.getPostComments(id).length
  return commentstxt === 1 ? `${commentstxt} comment` : `${commentstxt} comments`;
});
const compiledpostTemplate = Template7.compile(postTemplate);


function home (e, page) {
  if (page.name === 'home') {
    $$('#posts').html(compiledpostTemplate({posts}))
  } 
}

$$(document).on('page:init', home)
$$(document).on('page:reinit', home)
